function [LL, prior, transmat, mu, Sigma, mixmat,gamma] = ...
...
% orig funciton above had verbose = 1
previous_loglik = -inf;
loglik = 0;
converged = 0;
num_iter = 1;
...
while (num_iter <= max_iter) & ~converged
    % E step
    [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op,gamma] = ...
        ess_mhmm(prior, transmat, mixmat, mu, Sigma, data);
    %   if sum(isnan(postmix)) > 0
    %         test = sum(isnan(postmix));
    %   end
    ...
    if adj_mu || adj_Sigma
        ...
        %     if sum(isnan(Sigma2)) > 0
        %         test = sum(isnan(Sigma2));
        %     end
        if adj_mu
...
function [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op,gamma] = ...
    ess_mhmm(prior, transmat, mixmat, mu, Sigma, data)
...
% % added by laseaman@umich.edu
% function converged = em_converged(loglik, previous_loglik, thresh);
%     if abs(loglik-previous_loglik) < thresh
%         converged = true;
%     else
%         converged = false;
%     end
