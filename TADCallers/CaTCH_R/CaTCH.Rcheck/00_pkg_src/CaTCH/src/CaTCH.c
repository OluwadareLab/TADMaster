...
//      if(d1/((j1-i1+1)*(j1-i1)) > d2/((j2-i2+1)*(j2-i2+1)))   di=(x/v)/(d1/((j1-i1+1)*(j1-i1)));
//
//      //      else di=(x/v)/(d2/((j2-i2+1)*(j2-i2+1)));
              for(k=i1;k<=j1;k++)
...
SEXP catch(SEXP input)
{
	...
	for(i=0;i<nrow;i++){
		...
		//fprintf(stderr,"%d %d %d \n",(int)REAL(input)[i+0*nrow],(int)REAL(input)[i+1*nrow],(unsigned short) REAL(input)[i+2*nrow]);
	...
	//insulation = (float **) calloc(size,sizeof(float *));
        //for (i=0;i<size;i++) insulation[i] = (float *) calloc(size,sizeof(float));
...
/*not printint output file*///	fp = fopen("ncluster.dat","w");
/*not printint output file*///	if (!fp) Rf_error("Cannot open outpu file");
	PROTECT(ncluster=Rf_allocMatrix(REALSXP,ND+1,2));
	
/*not printint output file*///	for (i=0;i<=ND;i++) fprintf(fp,"%f\t%d\n",(float)i/ND,ncl[i]);
	for (i=0;i<ND+1;i++) {
		REAL(ncluster)[i+0*(ND+1)]=(float)i/ND;
		REAL(ncluster)[i+1*(ND+1)]=ncl[i];
	}
/*not printint output file*///	fclose(fp);

/*not printint output file*///	fp = fopen("cluster-plot.dat","w");
/*not printint output file*///	if (!fp) Rf_error("Cannot open output file");
/*not printint output file*/
/*	for (i=1;i<=ND;i++)
		for (j=0;j<ncl[i];j++)
		{
			fprintf(fp,"%f\t%d\t%d\n",(float)i/ND,cfrom[i][j],cfrom[i][j]);
			fprintf(fp,"%f\t%d\t%d\n",(float)i/ND,cfrom[i][j],cto[i][j]);
			fprintf(fp,"%f\t%d\t%d\n",(float)i/ND,cto[i][j],cto[i][j]);
			fprintf(fp,"%f\t%d\t%d\n",(float)i/ND,cto[i][j],cfrom[i][j]);
			fprintf(fp,"%f\t%d\t%d\n",(float)i/ND,cfrom[i][j],cfrom[i][j]);
			tot++;
		}*/
/*not printint output file*///	fclose(fp);
...
/*not printint output file*/
/*	fp = fopen("clusters.dat","w");
	if (!fp) Rf_error("Cannot open output file");
	for (i=1;i<=ND;i++)
		for (j=0;j<ncl[i];j++){
			fprintf(fp,"%f\t%d\t%d\n",(float)i/ND,cfrom[i][j],cto[i][j]);
			REAL(out)[appo+tot*0] =(float)i/ND;
